...
<article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
	...
	$image_url = ! post_password_required() ? get_the_post_thumbnail_url( get_the_ID(), 'zeroone-fullscreen' ) : '';
				...
				<div class="cover-color-overlay color-accent<?php echo esc_attr( $color_overlay_classes ); ?>"<?php echo $color_overlay_style; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- We need to double check this, but for now, we want to pass PHPCS ;) ?>></div>
					...
					<header class="entry-header has-text-align-center">
						<div class="entry-header-inner section-inner medium">
							 ...
							 */
							$show_categories = apply_filters( 'zeroone_show_categories_in_entry_header', true );
							...
							if ( is_page() ) {
								...
								<div class="to-the-content-wrapper">
									...
									<a href="#post-inner" class="to-the-content fill-children-current-color">
										<?php zeroone_the_theme_svg( 'arrow-down' ); ?>
										<div class="screen-reader-text"><?php _e( 'Scroll Down', 'zeroone' ); ?></div>
									</a><!-- .to-the-content -->
								...
								zeroone_the_post_meta( get_the_ID(), 'single-top' );
	...
	<div class="post-inner" id="post-inner">
		...
		wp_link_pages(
			array(
				'before'      => '<nav class="post-nav-links bg-light-background" aria-label="' . esc_attr__( 'Page', 'zeroone' ) . '"><span class="label">' . __( 'Pages:', 'zeroone' ) . '</span>',
				'after'       => '</nav>',
		...
		zeroone_the_post_meta( get_the_ID(), 'single-bottom' );
